# --- START OF MOBILE-OPTIMIZED VERSION ---
...
# ---------------------- Arabic Reshaper (only reshaping, no bidi) ----------------------
try:
    import arabic_reshaper
    HAS_RESHAPER = True
except ImportError:
    HAS_RESHAPER = False
    print("Warning: arabic-reshaper not installed. Arabic text may not connect correctly.")

def reshape_arabic(text):
    """Reshape Arabic string for proper display (connects letters)."""
    if not HAS_RESHAPER or not text or not isinstance(text, str):
        return text
    # Only reshape if text contains Arabic Unicode range
...
class ComboboxSearchable(ttk.Combobox):
    ...
    def __init__(self, master, **kwargs):
        if 'width' not in kwargs:
            kwargs['width'] = 18   # reduced for mobile
...
class ArrayFieldEditor(ttk.Frame):
    ...
    def __init__(self, parent, field_name, initial_list=None, dirty_callback=None, **kwargs):
        ...
        add_btn = ttk.Button(self, text="+ Add", command=self.add_entry, width=6)  # smaller button
    ...
    def add_entry(self, initial_value=""):
        ...
        entry = ttk.Entry(row_frame, width=25)  # reduced width
...
class MatchEditorTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # --- LEFT SIDE: LIST & BUTTONS (reduced width) ---
        left_frame = ttk.Frame(self, width=220, relief="sunken")
        ...
        ttk.Label(left_frame, text="Matches by Date", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, pady=3, sticky="ew")
        ...
        btn_frame.grid(row=2, column=0, pady=3)
        
        ttk.Button(btn_frame, text="New", image=self.icons.get('new'), compound=tk.LEFT, command=self.new_match).grid(row=0, column=0, padx=2, pady=1)
        ttk.Button(btn_frame, text="Dup", image=self.icons.get('duplicate'), compound=tk.LEFT, command=self.duplicate_match).grid(row=0, column=1, padx=2, pady=1)
        ttk.Button(btn_frame, text="Del", image=self.icons.get('delete'), compound=tk.LEFT, command=self.delete_match).grid(row=1, column=0, padx=2, pady=1)
        ttk.Button(btn_frame, text="Coll", image=self.icons.get('collapse'), compound=tk.LEFT, command=self.collapse_all_dates).grid(row=1, column=1, padx=2, pady=1)
        ttk.Button(btn_frame, text="Exp", image=self.icons.get('expand'), compound=tk.LEFT, command=self.expand_all_dates).grid(row=1, column=2, padx=2, pady=1)
        ...
        # --- Simple fields (reduced widths) ---
        simple_fields = [
            ("match_id", "ID:", "text"), ("group", "Grp:", "text"), ("week", "Wk:", "text"),
            ("date", "Date:", "date"), ("time", "Time:", "text"),
        ...
        for field, label, ftype in simple_fields:
            lbl = ttk.Label(scrollable_frame, text=label, width=8, anchor="e")
            lbl.grid(row=row, column=0, padx=2, pady=1, sticky="e")
            widget = None
            if ftype == "date":
                widget = DateEntry(scrollable_frame, width=12, date_pattern='yyyy-mm-dd')
                widget.bind("<<DateEntrySelected>>", self._mark_dirty)
            elif ftype == "venue_dropdown":
                widget = ComboboxSearchable(scrollable_frame, values=self.venue_list, width=20)
                ...
                widget = ttk.Combobox(scrollable_frame, values=values, width=18)
                widget.bind("<<ComboboxSelected>>", self._mark_dirty)
            else:
                widget = ttk.Entry(scrollable_frame, width=22)
                widget.bind("<KeyRelease>", self._mark_dirty)
            
            widget.grid(row=row, column=1, padx=2, pady=1, sticky="ew", columnspan=3)
            self.widgets[field] = widget
            row += 1
        
        # --- HOME vs AWAY header ---
        ttk.Label(scrollable_frame, text="HOME", font=("Segoe UI", 9, "bold"), foreground="blue").grid(row=row, column=1, sticky="w", padx=5, pady=(5,2))
        ttk.Label(scrollable_frame, text="AWAY", font=("Segoe UI", 9, "bold"), foreground="red").grid(row=row, column=3, sticky="w", padx=5, pady=(5,2))
        row += 1

        # --- Team row ---
        ttk.Label(scrollable_frame, text="Team:", width=8, anchor="e").grid(row=row, column=0, padx=2, pady=1, sticky="e")
        home_team_cb = ComboboxSearchable(scrollable_frame, values=list(self.team_map.keys()), width=20)
        home_team_cb.bind("<<ComboboxSelected>>", self._mark_dirty)
        home_team_cb.bind("<KeyRelease>", self._mark_dirty)
        home_team_cb.grid(row=row, column=1, padx=2, pady=1, sticky="ew")
        self.widgets["home_team_id"] = home_team_cb

        ttk.Label(scrollable_frame, text="Team:", width=8, anchor="e").grid(row=row, column=2, padx=2, pady=1, sticky="e")
        away_team_cb = ComboboxSearchable(scrollable_frame, values=list(self.team_map.keys()), width=20)
        away_team_cb.bind("<<ComboboxSelected>>", self._mark_dirty)
        away_team_cb.bind("<KeyRelease>", self._mark_dirty)
        away_team_cb.grid(row=row, column=3, padx=2, pady=1, sticky="ew")
        self.widgets["away_team_id"] = away_team_cb
        row += 1

        # --- Score row ---
        ttk.Label(scrollable_frame, text="Score:", width=8, anchor="e").grid(row=row, column=0, padx=2, pady=1, sticky="e")
        home_score_entry = ttk.Entry(scrollable_frame, width=22)
        home_score_entry.bind("<KeyRelease>", self._mark_dirty)
        home_score_entry.grid(row=row, column=1, padx=2, pady=1, sticky="ew")
        self.widgets["home_score"] = home_score_entry

        ttk.Label(scrollable_frame, text="Score:", width=8, anchor="e").grid(row=row, column=2, padx=2, pady=1, sticky="e")
        away_score_entry = ttk.Entry(scrollable_frame, width=22)
        away_score_entry.bind("<KeyRelease>", self._mark_dirty)
        away_score_entry.grid(row=row, column=3, padx=2, pady=1, sticky="ew")
        self.widgets["away_score"] = away_score_entry
        row += 1

        # --- Array fields with SHORT labels ---
        array_pairs = [
            ("home_squade", "away_squade", "SQ"),
            ("home_scorers", "away_scorers", "SCR"),
            ("home_yc", "away_yc", "YC"),
            ("home_rc", "away_rc", "RC"),
            ("home_sub", "away_sub", "SUB")
        ]
        ...
        for home_field, away_field, suffix in array_pairs:
            ttk.Frame(scrollable_frame, height=5).grid(row=row, column=0, columnspan=4)
            row += 1
            
            lbl_home = ttk.Label(scrollable_frame, text=f"H{suffix}:", width=6, anchor="e")
            lbl_home.grid(row=row, column=0, padx=2, pady=1, sticky="ne")
            
            editor_home = ArrayFieldEditor(scrollable_frame, home_field, dirty_callback=self._mark_dirty)
            editor_home.grid(row=row, column=1, padx=2, pady=1, sticky="nsew")
            self.widgets[home_field] = editor_home
            
            lbl_away = ttk.Label(scrollable_frame, text=f"A{suffix}:", width=6, anchor="e")
            lbl_away.grid(row=row, column=2, padx=2, pady=1, sticky="ne")
            
            editor_away = ArrayFieldEditor(scrollable_frame, away_field, dirty_callback=self._mark_dirty)
            editor_away.grid(row=row, column=3, padx=2, pady=1, sticky="nsew")
            self.widgets[away_field] = editor_away
            
            row += 1

        ttk.Button(scrollable_frame, text="Save", image=self.icons.get('save'), compound=tk.LEFT, command=self.save_current_match).grid(row=row, column=0, columnspan=4, pady=10)
    ...
    def refresh_match_tree(self):
        ...
        for date in sorted(matches_by_date.keys()):
            ...
            for match in sorted(matches_by_date[date], key=lambda m: m.get("time", "")):
                ...
                # reshape Arabic team names for tree display
                home_name_disp = reshape_arabic(home_name)
                away_name_disp = reshape_arabic(away_name)
                match_text = f"{match.get('match_id')} - {home_name_disp} vs {away_name_disp}"
    ...
    def populate_form(self, match):
        for field, widget in self.widgets.items():
            ...
            if field == "date":
                ...
                # display reshaped name in combobox
    ...
    def save_current_match(self):
        ...
        for field, widget in self.widgets.items():
            if field == "date": 
                ...
                # get original name from reshaped display
                display_name = widget.get()
                # find original name (unreshaped) from team_map keys
                original_name = None
...
class TeamEditorTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Label(left_frame, text="Teams", font=("Segoe UI", 10, "bold")).pack(pady=3)
        
        list_frame = ttk.Frame(left_frame)
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        self.team_listbox = tk.Listbox(list_frame, width=30, font=("Segoe UI", 9), relief="flat", borderwidth=1)
        ...
        self.widgets = {}
        fields = [
            ("team_id", "ID:", "text"), ("name", "Name:", "text"), ("group", "Grp:", "text"),
            ("logo", "Logo:", "text"), ("field", "Field:", "text"), ("fieldurl", "Field URL:", "text"),
            ("city", "City:", "text"), ("information", "Info:", "text_multiline"), ("point_deduction", "Pts Ded:", "int")
        ]
        
        row = 0
        for field, label, ftype in fields:
            ttk.Label(scrollable_frame, text=label, width=8, anchor="e").grid(row=row, column=0, padx=2, pady=2, sticky="ne")
            widget = None
            if ftype == "text_multiline":
                widget = tk.Text(scrollable_frame, width=40, height=3, relief="flat", font=("Segoe UI", 9))
                widget.bind("<KeyRelease>", self._mark_dirty)
            else:
                widget = ttk.Entry(scrollable_frame, width=30)
                widget.bind("<KeyRelease>", self._mark_dirty)
            
            widget.grid(row=row, column=1, padx=2, pady=2, sticky="w")
            self.widgets[field] = widget
            row += 1
            
        ttk.Label(scrollable_frame, text="Squad:", font=("Segoe UI", 9, "bold")).grid(row=row, column=0, columnspan=2, sticky="w", pady=(5,2))
        row += 1
        ...
        ttk.Button(scrollable_frame, text="Save", image=self.icons.get('save'), compound=tk.LEFT, command=self.save_current_team).grid(row=row, column=0, columnspan=2, pady=10)
    ...
    def refresh_team_list(self):
        ...
        for i, team in enumerate(self.sorted_teams):
            ...
            display_text = f"{team.get('team_id', '')} - {name_disp}"
    ...
    def save_current_team(self):
        ...
        for field, widget in self.widgets.items():
            if isinstance(widget, tk.Text): 
                ...
                value = widget.get().strip()
                # if name field, store the original (unreshaped) name? 
                # The user might have typed reshaped text? We'll assume they type original.
...
class MainApp:
    def __init__(self, root):
        ...
        # Mobile-friendly window size: fullscreen or 90% of screen
        ...
        self.toolbar = ttk.Frame(root, padding=3)
        self.toolbar.pack(side=tk.TOP, fill=tk.X)
        
        ttk.Button(self.toolbar, text="Open File", command=self.open_new_file).pack(side=tk.LEFT, padx=3)
    ...
    def setup_styles(self):
        ...
        self.default_font.configure(family="DejaVu Sans", size=9)  # Arabic-friendly font
        ...
        self.style.configure('TLabel', background=BG_COLOR, padding=2)
        self.style.configure('TButton', padding=3, borderwidth=1)
        self.style.configure("Treeview", rowheight=22, fieldbackground=BG_COLOR, font=("DejaVu Sans", 8))
    ...
    def refresh_helpers_and_ui(self):
        ...
        if hasattr(self, 'match_tab'):
            ...
            # update combobox with reshaped names for display
            reshaped_names = [reshape_arabic(name) for name in self.team_map.keys()]
            self.match_tab.widgets["home_team_id"].update_values(reshaped_names)
            self.match_tab.widgets["away_team_id"].update_values(reshaped_names)
...
# --- END OF MOBILE-OPTIMIZED VERSION ---