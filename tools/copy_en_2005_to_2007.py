...
dst = Path(r"c:\Users\waell\Documents\GitHub\FootballData\2007\A\A2007Data.json")
...
for t in dteams:
    ...
    if src_name_en:
        ...
        if ar_val: new_name['ar'] = ar_val
            ...
            # if no ar, try to keep existing en if present
...
print(f"Processed {len(dteams)} teams; updated {len(changed)} teams: {', '.join(changed)}")
